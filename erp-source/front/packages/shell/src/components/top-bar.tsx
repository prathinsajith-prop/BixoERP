...
function OrgSelector() {
  ...
  const handleSwitch = async (org: Org) => {
    if (!org.id || org.id === currentOrg?.id) {
      setOpen(false);
      return;
    }
    setSwitching(org.id);
    ...
    try {
      ...
      const data = (
        res as { data?: { data?: { accessToken?: string; tenantId?: string } } }
      ).data?.data;
      if (data?.accessToken) {
  ...
  const gradient =
    ORG_GRADIENTS[(gradientIdx >= 0 ? gradientIdx : 0) % ORG_GRADIENTS.length];
    ...
    <div className="relative" ref={ref}>
      <button
        ...
        style={
          open
            ? {
                outline: "2px solid var(--gogo-primary)",
                outlineOffset: "-2px",
              }
            : undefined
        }
      >
        <div
          className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br ${gradient} text-[11px] font-bold text-white`}
        >
          {initial}
        </div>
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-72 rounded-xl bg-white py-2 shadow-xl ring-1 ring-gray-200/60 dark:bg-gray-800 dark:ring-gray-700">
          <p className="mb-1 px-4 text-[10px] font-bold uppercase tracking-wider text-gray-400">
            Organizations
          </p>
          {switchError && (
            <div className="mx-2 mb-1 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700 dark:bg-red-900/30 dark:text-red-300">
              {switchError}
            </div>
          )}
          {orgs.map((org, idx) => {
              ...
              <button
                ...
                style={
                  isActive
                    ? {
                        backgroundColor:
                          "color-mix(in srgb, var(--gogo-primary) 8%, transparent)",
                      }
                    : undefined
                }
              >
                <div
                  className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br ${ORG_GRADIENTS[idx % ORG_GRADIENTS.length]} text-[11px] font-bold text-white`}
                >
                  {org.name?.charAt(0)?.toUpperCase() || "O"}
                </div>
                <div className="min-w-0 flex-1">
                  <p
                    className={`truncate text-sm font-medium ${isActive ? "dark:text-purple-200" : "text-gray-700 dark:text-gray-300"}`}
                    style={
                      isActive
                        ? { color: "var(--gogo-primary-dark)" }
                        : undefined
                    }
                  >
                    {org.name}
                ...
                {org.role && (
                  <span
                    className={`shrink-0 rounded-full px-1.5 py-px text-[9px] font-bold uppercase tracking-wide ${roleColors[org.role] || roleColors.Member}`}
                  >
                    {org.role}
                  </span>
                )}
          ...
          <a
            href="/organization"
            className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-600 transition hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-white/5"
          >
            <svg
              className="h-4 w-4"
              fill="none"
          ...
          <a
            ...
            className="flex items-center gap-2.5 px-3 py-2 text-sm font-medium transition hover:bg-gray-50 dark:hover:bg-white/5"
            style={{ color: "var(--gogo-primary)" }}
          >
            <svg
              className="h-4 w-4"
