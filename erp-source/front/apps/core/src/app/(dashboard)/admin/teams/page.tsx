...
export default function TeamsPage() {
  ...
  useEffect(() => {
    if (!orgId) return;
    setLoading(true);
    Promise.all([authApi.listTeams(orgId), authApi.listDepartments(orgId)])
      .then(([teamRes, deptRes]) => { setItems(teamRes.data?.data || []); setDepartments(deptRes.data?.data || []); })
      .catch(() => { setItems([]); setDepartments([]); })
      .finally(() => setLoading(false));
  }, [orgId]);

  const deptName = (deptId?: string) => departments.find((d) => d.id === deptId)?.name || '—';
  const resetForm = () => { setForm({ name: '', code: '', description: '', departmentId: '' }); setEditingId(null); setShowForm(false); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) { const res = await authApi.updateTeam(orgId, editingId, body); setItems(items.map((d) => (d.id === editingId ? res.data?.data : d))); }
      else { const res = await authApi.createTeam(orgId, body); setItems([...items, res.data?.data]); }
      resetForm();
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await authApi.deleteTeam(orgId, id); setItems(items.filter((d) => d.id !== id)); } catch { /* handled */ }
  };

  const startEdit = (item: Team) => { setForm({ name: item.name, code: item.code, description: item.description || '', departmentId: item.departmentId || '' }); setEditingId(item.id); setShowForm(true); };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mb-6 flex justify-end">
          <button onClick={() => { resetForm(); setShowForm(!showForm); }} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">+ Add Team</button>
        </div>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {items.map((item) => (
                    <tr key={item.id} className="group">
                      <td className="py-3 text-sm font-medium"><button onClick={() => router.push(`/admin/teams`)} className="text-blue-600 transition hover:text-blue-800 hover:underline dark:text-blue-400 dark:hover:text-blue-300">{item.name}</button></td>
