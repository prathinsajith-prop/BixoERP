  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (editingIdRef.current) {
        const res = await authApi.updateDepartment(orgId, editingIdRef.current, body);
        setItems(items.map((d) => (d.id === editingIdRef.current ? res.data?.data : d)));
      } else {
        const res = await authApi.createDepartment(orgId, body);
  ...
  const deptColumns: TableColumn<Department>[] = [
    {
      ...
      header: 'Name',
      render: (item) => (
        <button onClick={() => router.push('/admin/departments')} className="text-sm font-medium text-blue-600 transition hover:text-blue-800 hover:underline dark:text-blue-400 dark:hover:text-blue-300">
          {item.name}
        </button>
      ),
    },
    ...
    },
    {
      key: 'divisionId',
      header: 'Division',
      render: (item) => <span className="text-sm text-gray-500 dark:text-gray-400">{divisionName(item.divisionId)}</span>,
    },
    {
      key: 'description',
      header: 'Description',
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <input className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                  {...register('name')}
              ...
              <div>
                ...
                <input className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm uppercase shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                  {...register('code')}
                  placeholder="BE"
                  maxLength={50}
                />
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Division</label>
                <select className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                  {...register('divisionId')}
                >
            ...
            <div className="mt-3 flex gap-2">
              <button type="submit" disabled={isSubmitting || saving} className="rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">{saving ? 'Saving…' : editingId ? 'Update' : 'Create'}</button>
              <button type="button" onClick={resetForm} className="rounded-lg bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">Cancel</button>
            </div>
          </form>
        </div>
      )}
