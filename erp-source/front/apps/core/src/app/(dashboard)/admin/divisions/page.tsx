...
export default function DivisionsPage() {
  ...
  useEffect(() => {
    if (!orgId) return;
    setLoading(true);
    authApi.listDivisions(orgId).then((r) => setItems(r.data?.data || [])).catch(() => setItems([])).finally(() => setLoading(false));
  }, [orgId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        const res = await authApi.updateDivision(orgId, editingId, form);
        setItems(items.map((d) => (d.id === editingId ? res.data?.data : d)));
      } else {
        const res = await authApi.createDivision(orgId, form);
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await authApi.deleteDivision(orgId, id); setItems(items.filter((d) => d.id !== id)); } catch { /* handled */ }
  };

  const startEdit = (item: Division) => { setForm({ name: item.name, code: item.code, description: item.description || '' }); setEditingId(item.id); setShowForm(true); };
  ...
  return (
    <div className="space-y-6">
        <PageHeader title="Divisions" subtitle="Manage organizational divisions" />
        <div className="mb-6 flex justify-end">
          <button onClick={() => { resetForm(); setShowForm(!showForm); }} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">+ Add Division</button>
        </div>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {items.map((item) => (
                    <tr key={item.id} className="group">
                      <td className="py-3 text-sm font-medium"><button onClick={() => router.push(`/admin/divisions`)} className="text-blue-600 transition hover:text-blue-800 hover:underline dark:text-blue-400 dark:hover:text-blue-300">{item.name}</button></td>
