...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription('ERP Integration Microservice — Webhooks, External API Adapters, Integration Logs')
    .setVersion('1.0')
    .addBearerAuth()
    .addServer('/api/v1/integrations')
    .build();
  ...
  console.log(`integration-svc running on port ${port}`);
}
