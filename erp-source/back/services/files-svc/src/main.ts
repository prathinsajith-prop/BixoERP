...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription('ERP Files Microservice — File Upload/Download, S3/MinIO Storage, Versioning, Access Control')
    .setVersion('1.0')
    .addBearerAuth()
    .addServer('/api/v1/files')
    .build();
  ...
  console.log(`files-svc running on port ${port}`);
}
