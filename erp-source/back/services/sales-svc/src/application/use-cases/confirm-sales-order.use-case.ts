...
export interface ConfirmSalesOrderOutput {
  id: string;
  orderNumber: string;
  status: string;
  confirmedAt: string;
...
export class ConfirmSalesOrderUseCase {
  ...
  async execute(input: ConfirmSalesOrderInput): Promise<ConfirmSalesOrderOutput> {
    ...
    // Domain rule: cannot confirm an already-confirmed order, must be DRAFT
    order.confirm();

    const saved = await this.orderRepo.saveWithOutbox(order);

    // Index in Elasticsearch on confirm
    ...
    // sales.order.confirmed → inventory-svc, mfg-svc, apar-svc, report-svc, audit-svc
